...
function outrutProductCards (productCards) {
...
const getArray = productCards.reduce((acc, product) => [...acc, {[product.name]: product.description}], []);
...
const getCard = () => {
  ...
  if (selectedCount >= 1 && selectedCount <= productCards.length) {
    outrutProductCards(productCards.slice(0, selectedCount))
  } else {
